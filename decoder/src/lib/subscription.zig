...
test "getKey" {
    ...
    var subscription = Subscription.init(1, 1, 6, std.mem.sliceAsBytes(root_hashes));
...
test "getKey largest range" {
    ...
    var subscription = Subscription.init(1, 0, std.math.maxInt(u64), std.mem.sliceAsBytes(root_hashes));
    try std.testing.expectEqualDeep(&[_]RootPosition{
        .{ .offset = 0, .power = 64 },
...
test "getKey single value range" {
    ...
    var subscription = Subscription.init(1, 4, 4, std.mem.sliceAsBytes(root_hashes));
    try std.testing.expectEqualDeep(&[_]RootPosition{
        .{ .offset = 4, .power = 0 },
    }, subscription.roots[0..1]);
...
test "getKey most suboptimal" {
    ...
    var subscription = Subscription.init(1, 1, std.math.maxInt(u64) - 1, std.mem.sliceAsBytes(root_hashes));
