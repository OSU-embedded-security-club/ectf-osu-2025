...
We will start by making a few mathematical definintions for the concepts of roots and trees.
...
We will prove that any interval in a tree of height $n$ can be encoded by a tree with at mosts $2n - 2$ nodes.
...
Timestamps are $64$ bit unsigned integers, so we choose $n=64$. The #link(<theorem>, [Theorem]) shows that no more than $2(64)-2=126$ roots are required to encode any interval from $[0, 2^64-1]$.
